...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    try {
      await saveDailyBlogLastRun({
        requestId,
        actor: 'system:cron',
        result
      })
    } catch (saveError) {
      console.error('[cron][daily-blog][save-last-run]', requestId, saveError)
    }

    console.info('[cron][daily-blog]', {
      requestId,
    ...
    console.error('[cron][daily-blog]', requestId, error)
    return fail(requestId, 500, 'INTERNAL_ERROR', 'Failed to run daily blog cron')
  }
}