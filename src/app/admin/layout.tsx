...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  return (
    <div className='relative min-h-screen pb-10' data-theme-scope='admin'>
      ...
      <header className='mx-auto flex w-full max-w-6xl flex-wrap items-center justify-between gap-4 px-5 pt-7 pb-5 sm:px-8'>
        <div>
          <h1 className='font-title text-3xl text-[var(--color-ink)]'>MLog Admin</h1>
          <p className='text-sm text-[var(--color-ink-soft)]'>前台编辑与发布后台</p>
        </div>

        <nav className='flex flex-wrap items-center justify-end gap-x-3 gap-y-2 text-sm text-[var(--color-ink-soft)]'>
          ...
          <Link href='/admin/media' className='transition hover:text-[var(--color-ink)]'>
            媒体库
          </Link>
