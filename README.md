...
- Auto flow:
  ...
  4. publish with existing admin publish pipeline (`mode=publish`) and auto-attach fixed tags `ai-auto`, `github-hot`
  5. EN content is auto-generated by existing translation flow
  6. after merge, trigger Vercel deploy hook (if `VERCEL_DEPLOY_HOOK_URL` is configured)
...
- Controls:
  ...
  - last-run snapshot (status, reason, selected repo, slug, fixed tags, deploy trigger result)
- Presets:
